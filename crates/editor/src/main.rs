  ...
  fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
    egui::SidePanel::left("left_panel")
      ...
      .show(ctx, |ui| {
        ...
        let response = egui::ScrollArea::vertical().show(ui, |ui| self.tree.ui(ui, &self.node));
        if response.inner {
            ...
            WzObjectType::Property(v) => match v {
              ...
              WzSubProperty::Sound(sound) => {
                ...
                if ui.button("Play").clicked() {
                  let source = Decoder::new(Cursor::new(sound.get_buffer())).unwrap();
              ...
              WzSubProperty::PNG(v) => {
                ...
                let ctx = ui.ctx();
                let handle = ctx.load_texture("tile", color_image, Default::default());
                ui.image(&handle);
                ui.add_space(8.0);
                if ui
                  ...
                  .clicked()
                {
                  ...
                  let filename = std::env::temp_dir()
                    .join(format!("{}.png", node.get_full_path().replace("/", "_")));
                  // 保存 PNG 文件到 tmp 目录
                  let image_rgba = image.clone().into_rgba8();
                  let _ = image_rgba.save(&filename);
                  // 使用 set_files 设置文件路径，这样粘贴时可以带上路径信息
                  let _ = self
                    .clipboard
                    .set_files(vec![filename.to_string_lossy().to_string()]);
                }
              }
