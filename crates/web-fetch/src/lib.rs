...
extern "C" {
  ...
  fn emscripten_fetch(
    attr: *const EmscriptenFetchAttributes,
    url: *const c_char,
  ) -> *mut EmscriptenFetch;
  ...
  if !user_data.is_null() {
    ...
    let data = if !fetch_ref.data.is_null() && fetch_ref.num_bytes > 0 {
      ...
      if actual_size > MAX_RESPONSE_SIZE {
        // 如果响应太大，发送错误
        let sender: Box<oneshot::Sender<FetchResult<FetchResponse>>> =
          Box::from_raw(user_data as *mut oneshot::Sender<FetchResult<FetchResponse>>);
        let _ = sender.send(Err(FetchError::NetworkError(format!(
      ...
      let mut data = match Vec::<u8>::try_reserve_exact(&mut Vec::new(), actual_size) {
        ...
        Err(_) => {
          ...
          let sender: Box<oneshot::Sender<FetchResult<FetchResponse>>> =
            Box::from_raw(user_data as *mut oneshot::Sender<FetchResult<FetchResponse>>);
          let _ = sender.send(Err(FetchError::NetworkError(format!(
            "Failed to allocate {} bytes for response",
            actual_size
          ))));
          emscripten_fetch_close(fetch);
          return;
  ...
  pub fn fetch(url: &str, options: Option<FetchOptions>) -> FetchFuture {
    #[cfg(target_os = "emscripten")]
    {
      ...
      unsafe {
        ...
        attr.attributes =
          EMSCRIPTEN_FETCH_LOAD_TO_MEMORY | EMSCRIPTEN_FETCH_PERSIST_FILE | EMSCRIPTEN_FETCH_APPEND;
        attr.timeout_msecs = options.timeout_ms.unwrap_or(60000 * 30);
        ...
        if fetch.is_null() {
          let sender: Box<oneshot::Sender<FetchResult<FetchResponse>>> =
            Box::from_raw(attr.user_data as *mut oneshot::Sender<FetchResult<FetchResponse>>);
