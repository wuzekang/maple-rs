...
use ::ui::{lazy, fragment, text, view, Image, IntoElement, View};
...
fn async_image_view(
    ...
    // 在同步作用域中获取 context
    let reader = use_context::<WzSplitReaderContext>()
            ...
            async move {
                if let Some(reader) = reader {
                    ...
                    match reader.get(&path).await {
                        Ok(node_handle) => {
                            // 从 NodeHandle 提取图片
                            match extract_image_from_handle(&node_handle).await {
                                Ok(image) => {
                                ...
                                Err(_) => {
                                    LoadState::Failed("Failed to extract image".to_string())
                                }
                            }
                        }
                        Err(_) => {
                            LoadState::Failed("Failed to load image".to_string())
                        }
                    }
                } else {
...
// 辅助函数：从 NodeHandle 提取图片
async fn extract_image_from_handle(handle: &wz_splitter::reader::NodeHandle) -> Result<DynamicImage, ()> {
    use wz_parser::WzNodeCast;
    
    // 获取原始节点
    let node = handle.raw_node();
    let node_read = node.read().unwrap();
    
    // 尝试提取 PNG 数据
    if let Some(png) = node_read.try_as_png() {
        png.extract_png().map_err(|_| ())
    } else {
        dbg!("error");
        Err(())
    }
