...
use web_fetch::{FetchError, FetchOptions, WebFetch};
...
pub async fn resolve_base_with_config(_config: &WzConfig) -> Result<Node, std::io::Error> {
  #[cfg(all(target_arch = "wasm32", target_os = "emscripten"))]
  {
        ...
        Ok(node) => {
          ...
          for wz_name in files_to_load {
            ...
            load_futures.push(async move {
              if let Err(e) =
                load_additional_wz_from_url(&base_node, &wz_name_clone, &wz_url, &config).await
              {
    ...
    let possible_paths = ["/Data/Base.wz", "Data/Base.wz", "./Data/Base.wz", "Base.wz"];
...
#[cfg(all(target_arch = "wasm32", target_os = "emscripten"))]
async fn load_additional_wz_from_url(
  base_node: &WzNodeArc,
  wz_name: &str,
  url: &str,
  config: &WzConfig,
    ...
    Ok(response) => {
      if response.status != 200 {
        return Err(std::io::Error::new(
          ...
          format!(
            "HTTP error for {}: {} {}",
            wz_name, response.status, response.status_text
          ),
        ));
      }
...
#[cfg(all(target_arch = "wasm32", target_os = "emscripten"))]
pub async fn resolve_base_from_url_with_config(
  url: &str,
  config: &WzConfig,
  ...
  match WebFetch::fetch(url, Some(options)).await {
    Ok(response) => {
      ...
      let wz_node =
        wz_parser::util::resolve_base_from_bytes(&response.data, None).map_err(|e| {
          std::io::Error::new(
            std::io::ErrorKind::InvalidData,
